...
test('defaults with no flags', () => {
  expect(parseFlags([])).toEqual({ dryRun: false, skipMaps: false, maxzoom: 15, splitAreas: false })
})
test('--dry-run and --skip-maps', () => {
  expect(parseFlags(['--dry-run', '--skip-maps'])).toEqual({ dryRun: true, skipMaps: true, maxzoom: 15, splitAreas: false })
})
test('--maxzoom sets a custom value', () => {
  expect(parseFlags(['--maxzoom', '12'])).toEqual({ dryRun: false, skipMaps: false, maxzoom: 12, splitAreas: false })
})
test('--split-areas sets splitAreas true', () => {
  expect(parseFlags(['--split-areas'])).toEqual({ dryRun: false, skipMaps: false, maxzoom: 15, splitAreas: true })
})
