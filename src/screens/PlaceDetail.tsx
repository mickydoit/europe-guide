...
export function PlaceDetail() {
  ...
  const ticketLabel = booking ? ticketLinkLabel(effectiveStatus(booking, undefined), attached.has(booking.id)) : null
    ...
    <main className="screen">
      ...
      <article className="place-detail" aria-label={name}>
        ...
        <div className="place-detail__actions">
          ...
          {booking && <Link className="btn--text" to={`/ticket/${content.trip.slug}/${booking.id}?trip=${content.trip.slug}`}>{ticketLabel}</Link>}
        </div>
        {(walkNext || route) && (
